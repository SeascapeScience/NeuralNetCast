...
speciesLoad <- function(fname = '../Data/CWM_yearsub.csv', cleanup=T)
{
  data <- read.csv(fname)
  if (cleanup) {
...
getIndices <- function(labelinfo = NA, trainoption = NA, randomize=F)
{
  if (randomize) {
    r <- runif(length(labelinfo))
    tr <- which(r>=trainoption)
    te <- which(r<trainoption)
  } else {
    tr <- which(labelinfo==trainoption)
    te <- which(labelinfo!=trainoption)
  }
