...
setwd('~/Work/Research/DTU/FishCast/Code')
...
#
#
# EXAMPLE WITH NORTH SEA DATA
source('NeuralNetCast01.4.R')
data <- speciesLoad('../Data/NorthSea.csv',cleanup = T,cleanupindex = 8)
data[is.na(data)]<-0 # Do I need to do this?
#data[is.nan(data)]<-0 # Do I need to do this?
#data[,8:ncol(data)] <- log10(1+data[,8:ncol(data)])
# Pull just one quarter (season)
data <- data[data$Quarter==1,]
# Calculate diversity index to use as label
#H <- matrix(NA,nrow=dim(data)[1])
#for (i in 1:length(H)) {
#  Pi <- data[i,8:ncol(data)]/sum(data[i,8:ncol(data)],na.rm = T)
#  H[i] <- -sum(Pi*(log(Pi)),na.rm=T)
#}
H<- data$Clupea.harengus #just for now

# Arrange the data that goes into the NN
im <- arrangeImages(data = data[,8:ncol(data)],
                    times = data$Year,
                    areas = data$SubArea,
                    labelcol = H,#data$ctyp,
                    ntimes = 5)#,
...
# Divide into training and testing indices
index <- getIndices(im$labeltime, trainrandom = .6, randomize = T)
#index <- getIndices(im$labeltime,testoption = 2018, randomize = F)
...
# EXAMPLE WITH GULF OF MAINE LANDINGS (not working)
